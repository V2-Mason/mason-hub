...
P2 — UX 持续优化:
- [ ] 根据 system_feedback 表持续迭代（EMP_0001）— 2026-03-25 巡检：连续3次无新反馈（3/12→3/24→3/25），静默期确认。建议改为事件驱动触发（新反馈入库/开店运营/Mason手动）。2条待深入(加载速度+UI美观度)维持P2等运营启动
...
> - 晚高峰 08:00-10:00 ET（= 中国 20:00-22:00）→ A 号
> - 午休 00:00-01:30 ET（= 中国 12:00-13:30）→ B 号
...
**EMP_0015 数据分析师 — 反馈闭环建设（2026-03-11 Mason 确认分阶段）**:

> 核心目标：闭合内容管线反馈环（生产→发布→数据→分析→策略调整→下一轮生产）
> 实施原则：先手动验证再自动化，Mason 协调节奏
> ⏰ 提醒：2026-03-18 提醒 Mason 推进此项

Phase 0 — Mason 手动验证（等 Mason 有店铺后台数据后启动）:
- [ ] 店铺后台数据采集方案确认 — Mason 提供后台数据（曝光/CTR/商品点击/进店/成交），确认哪些指标有用（Mason）
- [ ] 手动跑一轮分析 — Mason 手动记录 2 周数据，验证四维框架哪些维度真正有用（Mason + EMP_0015）

Phase 1 — 自动化采集（手动验证通过后）:
- [ ] 后台数据采集管道 — EMP_0014 建管道，定时采集店铺后台核心指标（EMP_0014）
- [ ] 数据落库 + data_catalog 注册 — 新数据集 raw_xhs_dashboard 注册（EMP_0014）

Phase 2 — 自动化诊断（采集稳定后）:
- [ ] 漏斗计算脚本 — 自动计算曝光→成交各步转化率（EMP_0015）
- [ ] 周度诊断报告生成 — 漏斗健康 + 内容效果 + 搜索卡位 + 竞品对比（EMP_0015）
- [ ] EMP_0015 cron 注册 — 日/周/月三个定时任务上线（EMP_0004）

Phase 3 — 闭环打通（诊断稳定后）:
- [ ] 诊断报告 → Slack 推送（EMP_0015 → #rednote）
- [ ] 策略决策模板 — Mason 每周 15 分钟决策，记录回写 backlog（Mason）
