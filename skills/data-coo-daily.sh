...
echo "--- 1. Risk Alerts ---"
RISK_RESPONSE=$(timeout "$TIMEOUT" curl -s "$SRX_API/api/intelligence/risk-alerts" 2>/dev/null || echo "ERROR")
...
SALES_RESPONSE=$(timeout "$TIMEOUT" curl -s "$SRX_API/api/sales/summary" 2>/dev/null || echo "ERROR")
...
DASH_RESPONSE=$(timeout "$TIMEOUT" curl -s "$SRX_API/api/dashboard/overview" 2>/dev/null || echo "ERROR")
