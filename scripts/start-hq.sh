...
tmux send-keys -t "$SESSION:0.0" "$ENV && cd $PROJECT_DIR && claude -p '读取 agents/EMP_0000.md，你是 Meta Manager。遵守该文件中的所有职责定义和行为边界。启动后说一句简短的状态报告。'" Enter

# 左右分屏，pane 1: Platform Dev（右侧）
tmux split-window -h -t "$SESSION:0.0" -c "$PROJECT_DIR"
tmux send-keys -t "$SESSION:0.1" "$ENV && cd $PROJECT_DIR && claude -p '读取 agents/EMP_0002.md，你是 Platform Dev。遵守该文件中的所有职责定义和行为边界。启动后说一句简短的状态报告。'" Enter

# pane 0 上下分屏，pane 2: SRE（左下）
tmux split-window -v -t "$SESSION:0.0" -c "$PROJECT_DIR"
tmux send-keys -t "$SESSION:0.2" "$ENV && cd $PROJECT_DIR && claude -p '读取 agents/EMP_0004.md，你是 SRE Agent。遵守该文件中的所有职责定义和行为边界。运行 uptime、df -h、free -h、systemctl status slack-bot，给出简短健康报告。'" Enter
