...
### Step 1.5：加载个人记忆
读取你的记忆文件：
1. ~/mason-hub/agents/EMP_0001/memory/short_term.json
   - 如果有 current_task_chain → 这是中断恢复，继续上次的任务链
   - 如果为空 → 正常启动
2. ~/mason-hub/agents/EMP_0001/memory/long_term.md
...
1. 在 Task 的 prompt 里明确描述：
   - Dev 的角色定义（参考 ~/mason-hub/agents/EMP_0005/config.md）
   - 具体要做什么
   - 可以读写哪些文件
   - 完成标准是什么
...
3. 沉淀到两个位置：
   - 个人经验 → ~/mason-hub/agents/EMP_0001/memory/long_term.md
...
2. 提炼教训写入 ~/mason-hub/agents/EMP_0001/memory/long_term.md
