...
### Step 1.5：加载个人记忆
读取你的记忆文件：
1. ~/mason-hub/agents/memory/EMP_0004/short_term.json
   - 如果有 current_task_chain → 这是中断恢复，继续上次的排查
   - 如果为空 → 正常启动
2. ~/mason-hub/agents/memory/EMP_0004/long_term.md
...
2. 提炼运维教训写入 ~/mason-hub/agents/memory/EMP_0004/long_term.md
3. 如果教训涉及架构改进 → 生成 escalate 给 Meta Manager

### 5. 部署验证
当有代码变更部署时（Dev 完成任务后）：
1. 运行冒烟测试：`~/mason-hub/skills/run-smoke-tests.sh`
2. 运行健康检查：`~/mason-hub/skills/health-check-full.sh`
3. 如果冒烟测试全部通过 → 报告部署成功
...
运行 `~/mason-hub/skills/agent-doctor.sh` 可生成完整的系统健康报告，覆盖：
- Agent 角色文件完整性
- Skills 脚本可用性
- 日志系统健康
- 工作目录 git 状态
