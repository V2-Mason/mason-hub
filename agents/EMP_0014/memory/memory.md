...
- 系统能力缺失 → 触发动作: Gateway 超时 60s 不足以完成完整 XHS 分析管道，需要 EMP_0002 增加超时或改为异步执行
- 系统能力缺失 → 触发动作: 管道脚本应统一用 `TZ='America/New_York'` 生成文件名，避免跨时区命名不一致
...
### 发现
- 5 个 XHS 脚本各有 `parse_count` 重复实现
- 这些脚本运行在阿里云，无法直接 `from data.tools import metrics`，所以 xhs-clean.py 必须内联指标函数
- 解决方案：先并行产出 clean JSON，验证一致性后再切 analysis 输入源
- `_xhs_analyze_compare.py` 的 `engage_rate`/`save_rate` 是内联计算，精度与 metrics.py 一致但风格不同

### Gap: 文档更新 → 已更新 data_catalog.yaml + 3 个 schema 文件
### Gap: 系统能力缺失 → 已修复 (2026-03-11)
...
- xhs-analyze-viral.py 新增 `--clean-json` 参数 + `analyze_from_clean()` 函数
- Step 0 成功 → Step 1 用 `--clean-json`；Step 0 失败 → 降级回原 SQLite 直读
- data_catalog.yaml 更新：clean_xhs_notes status: planned → active
- TZ='America/New_York' 加到 clean 步骤

### 发现
- clean JSON 缺 content_tier 字段，analyze_from_clean 用 setdefault 补全
- 55 个已有测试全部通过
- 向后兼容设计正确

### Gap: 纯知识
- 下次实际 cron 触发时需确认阿里云端到端跑通
- content_tier 字段长期应由 clean 层产出

## Lesson: 素仁轩历史销售快照管道 (2026-03-11)

### 做了什么
- srx-snapshot.py：调 3 个 API 端点写入 SQLite 时序表
- srx_sales_snapshot.yaml schema 定义
...
### 发现
- API 返回嵌套在 `data` 键下
- dashboard API 返回非常丰富：库存按品牌/品类分布、临期预警（10 个产品 expiry_critical）
- 当前 revenue=0、orders=0 — 素仁轩还没正式开售，但库存数据已有价值

### Gap: 配置错误 → 已修
### Gap: 纯知识
- srx-snapshot.py 需注册 cron（每日 20:00 ET）— EMP_0004 负责

## Lesson: 数据健康检查自主修复 (2026-03-11)

### 做了什么
- 运行 data_health_check.sh 发现 14/17 健康，1 黄 2 红
- 修复 clean_xhs_notes ❌：SCP xhs-clean.py 到阿里云执行，产出 1226 条清洗数据
- 修复 report_optimization ❌：health check 脚本对 YYYY-MM-DD 模式的周报只查今天/昨天文件
- 修复后 16/17 健康，1 黄

### 发现
- clean_xhs_notes 虽然代码就绪，但还从未被实际触发过
- data-sync.sh 的 mirror 目录未同步 clean/ 子目录
- health check 的 YYYY-MM-DD fallback bug 影响所有低频数据集

### Gap
- 配置错误 → 已修
- 系统能力缺失 → data-sync.sh 应同步 clean/ 子目录到 mirror

## Lesson: 例行健康检查 (2026-03-12)

### 做了什么
- 运行 data_health_check.sh：16/17 健康，1 黄，2 跳过
- raw_scout_intel 持续 4 天未更新，符合升级规则（连续 2 次检查未恢复）
...
### Gap: 纯知识
- raw_scout_intel 非 EMP_0014 职责，按协议已标记升级
→ 已更新 (2026-03-12): 根因定位并修复，见下方 Lesson

## Lesson: Scout v2 LLM 模型名修复 + data_catalog 路径对齐 (2026-03-12)

### 做了什么
- 根因定位：DashScope LLM 调用失败（404: model `deepseek-chat` does not exist）
- 修复 intel/engines/config.yaml：`model: deepseek-chat` → `model: deepseek-v3`
- 手动跑通完整 pipeline，73 条 intel items
- 修复 data_catalog.yaml 路径错误
- 修复后健康检查全绿 17/17

### 发现
- DashScope 模型名与 DeepSeek 官方不同：官方 `deepseek-chat`，DashScope 是 `deepseek-v3`
- Scout v2 cron 因 LLM 404 一直静默失败（无 log 文件 = 无人察觉）

### Gap
- 配置错误 → 已修（model name + catalog path）
- 纯知识 → DashScope 模型名映射：deepseek-v3, deepseek-r1, qwen-plus 等
- 系统能力缺失 → Scout v2 cron 应写 log 到 logs/scout-v2.log

## Lesson: 例行健康检查 — 全绿 (2026-03-12 #2-8)

- 连续 8 次全绿（17/17 健康，2 跳过），管道进入稳定运行期
- 总数据量 18MB，远低于 50MB 方案 C 升级阈值

## Lesson: XHS 主干管道标准接口改造 (2026-03-12)

### 做了什么
- 新增 `data/tools/pipeline.py`：管道状态查询 + 数据装配
- 新增 `data/pipelines/assemble-data.py`：CLI 工具替代 bash 手工文件读取
- 改造 `optimization-cycle.sh`：Step 1 从 120 行精简为 1 行 SDK 调用
- SDK __init__.py 版本升级 0.1.0 → 0.2.0

### 发现
- 改造后 SDK 统一处理 fallback 逻辑，bash 只消费 JSON
- 变量名保持向后兼容

### Gap: 纯知识
- 下次消费者改造目标：EMP_0008/EMP_0015 也应调 SDK 而非直读文件
