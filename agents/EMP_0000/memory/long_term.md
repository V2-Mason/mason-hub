...
### v1 运行数据（13 天：2/27-3/12）
- 16 个 agent，long_term.md 总计 1862 行
- Top 4: EMP_0008 (336行)、EMP_0014 (300行)、EMP_0003 (209行)、EMP_0002 (183行)
- Bottom 3: EMP_0011 (7行)、EMP_0007 (13行)、EMP_0013 (19行)
- short_term.json：14/16 有文件，实质使用率低
- 语义搜索：ChromaDB 1.9MB，run-agent.sh 启动时自动注入
- 未解决 🏗️ gap: 3 条（EMP_0014: Gateway 超时/TZ 统一/clean 同步）

### 已验证优势
- 独立记忆 + 启动注入 = 无冲突、无污染
- Lesson gap 分类 → 问题可追踪
- 语义搜索跨 agent 提供上下文（虽然命中率待验证）

### 已暴露痛点（按严重程度）
1. **记忆膨胀无控制** — EMP_0014 一天写 8 条几乎相同的"全绿"lesson（300 行中有效<150），EMP_0008 增速最快
2. **知识重复** — parse_count() 在 5 个文件重复定义；多 Agent 并行经验在 3 处重复
3. **跨 agent 不流通** — EMP_0014 发现 data-coo-daily.sh 的 data 键 bug，EMP_0002 不知道
4. **gap 跟进断裂** — 🏗️ gap 写了但无人 triage（EMP_0012 应负责但无触发机制）

### 决策：暂不启动 v2，但标记 3 个 v1.5 改进
- v2（共享知识自动沉淀）的前提是大量跨 Agent 协作数据，当前只有 1 次（视频管线 4-Agent）
- 过早抽象违反 Mason 原则
- 自治闭环仅运行 2 天，数据不足

### v1.5 改进建议（可由现有 agent 执行，不需架构变更）
1. **Lesson 压缩规则** — 连续相同结果的 lesson 合并为 1 条 + 计数（如"全绿 ×8"），控制 long_term.md < 300 行（EMP_0002 改收工流程）
2. **Gap Triage 自动化** — Dispatcher 每日扫描 🏗️/🔗 未修标记，自动通知对应 owner（EMP_0002 写脚本）
3. **跨 Agent 经验广播** — /commit 时检测 lesson 中的跨 agent 影响（如"EMP_0014 发现 X 影响 EMP_0002"），自动 append 到对方 short_term.json（EMP_0002 改 /commit skill）

### v2 触发条件（不变）
- 任一 agent long_term.md > 500 行
- 出现 3 次以上"agent A 踩了 agent B 已踩过的坑"
- Mason 主动要求
- Gap 分类：📚 纯知识 → 留存

### L3a/L3b 拆分框架 (2026-03-12)
- L3 层混了两种不同性质：L3a 决策型（需要 Mason 判断力，可通过经验积累降级）vs L3b 执行型（需要 Mason 的手，做一次就清掉的一次性门槛队列）
- 降级条件按任务类型区分：可观测结果任务用成功率自动降级；主观判断任务用置信度+抽检模式
- 实际应用：61 个 L3 任务中 10 个是 L3b 一次性动作，其中 5 个 <5 分钟可完成
- Gap 类型：设计缺口 — 四层模型的粒度在 L3 层不够细
