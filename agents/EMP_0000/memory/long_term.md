...
## 记忆系统 v2 评估 (2026-03-12)

### v1 现状数据
- 16 个 agent，long_term.md 总计 1854 行
- 最大 EMP_0008 (328行)，最小 EMP_0011 (7行)
- short_term.json 使用率低（仅 3/16 有实质内容）
- 未解决 🏗️ gap: 3 条
...
### 已暴露痛点
1. 知识重复：同一经验写 3 处（EMP_0000/EMP_0009/global），改一处忘改其他
2. 跨 agent 不流通：A 的经验 B 不知道，靠人工搬运
3. 记忆膨胀：EMP_0008 已 328 行，按当前增速 2 月后超实用上限

### 决策：暂不启动 v2 设计
- 痛点 1/2 当前人工可管理（总量 1854 行）
- 痛点 3 有 1-2 月缓冲
- 自治闭环仅运行 2 天，数据不足以判断哪些经验真正需要跨 agent 共享
- 过早抽象违反 Mason 原则
