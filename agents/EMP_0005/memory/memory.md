...
### 管道组件
- `skills/xhs/xhs-cookie-check.sh`: Cookie 有效性检测（SSH→阿里云→XHS API），过期自动 Slack 通知
- `skills/xhs/xhs-crawl.sh --task 1~4`: 采集调度器，先 cookie 检查再跑 MediaCrawler
- `skills/xhs/xhs-analyze.sh`: 数字归一化 + 假流量过滤 + 互动评分 + 爆帖 Top20 + 关键词统计 → JSON
- `skills/xhs/xhs-strategy-briefing.sh`: 规则策略推荐（3 号各自内容建议）→ JSON + Slack 摘要
